...
const buttonMakers = [
 ...
 return (
    <figure className={`${isNavOpen?"fixed flex":"hidden"} min-h-full z-50 w-[20rem] md:w-[15rem] md:static md:flex flex-col gap-y-[2rem] md:p-[2rem] px-[1rem] py-[2rem] transparent-cards`}>
      {/* cancel bar btn + logo */}
      <div className="flex gap-x-4 items-center">
        <button className={`${isNavOpen && "block"} md:hidden cursor-pointer`} onClick={()=>{setIsNavOpen(!isNavOpen)}}>
      <ImCancelCircle size={'1.5rem'} className="hover:text-red-950" />
        </button>
